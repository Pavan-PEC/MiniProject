...
class Att_MVAHN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkvc = qkvc.contiguous().view(b, self.num_patch*self.num_patch, self.inner_dim * 4) # For converting the shape
        # qkvc-->[b, num_patch*num_patch, inner_dim*4]

        qkvc = qkvc.chunk(4, dim=-1)
        # qkvc-->q, k, v, c-->[b, num_seq, inner_dim=head_dim * num_heads]

        q, k, v, c = map(lambda t: einops.rearrange(t, "b n (h d) -> b h n d", h=self.num_heads), qkvc) # Calc of q, k v, c
...
class Encoder_MVAHN(nn.Module):
    #num_heads is no of attention scores
